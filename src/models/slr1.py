    ...
    def generate_automaton(self, name=''):
        ...
        for r in range(len(self.C)):
            for c in self.parse_table_symbols:
                ...
                elif self.parse_table[r][c] == 'accept':
                    automaton.node('accept', '<<B>ACCEPT</B>>', shape='none')
                    automaton.edge(f'I{r}', 'accept', label='$')

        # Renderizar y guardar el autómata en un directorio específico
        automaton.render(filename=f'LR0_{name}', directory="./src/lr0_out/",cleanup=True, format='png', view=False)
        print('SUCCES: LR(0) Automaton generated, output saved in /src/lr0_out/ folder.')
